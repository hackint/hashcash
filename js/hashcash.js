...
function guessHash(hashfun, rounds, salt, first, max, payload, onSuccess) {
    var lastdate;
    lastdate = new Date();
	...
	if ((cur - first) % 1000 == 0) {
		...
		minutestring = minutes.toString() + " minutes";
	    }
	    postMessage({"event": "progress",
			 "text": (cur-first).toString() + " variants tried, max time left: ~" + minutestring});
	}
	var str = salt + cur.toString();
	var hash = str;
